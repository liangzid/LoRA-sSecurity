...
def main_plot1():
    ...
    row_dict = {
        "sst2": "SST-2 (Poisoning PR=0.3)",
        "cola": "COLA",
        "qnli": "QNLI (Poisoning PR=0.3)",
        "qqp": "QQP",
    }
    column_ls = [
        "Accuracy",
    ...
    for i_row, row in enumerate(row_ls):
        ...
        axs[i_row].set_xlabel(
            "Initialization Variance", fontsize=font_size-5)
        axs[i_row].set_title(
    ...
    # plt.show()
    plt.savefig("./hyprid_varyinit_acc1x4.pdf", pad_inches=0.1)
    print("Save done.")
